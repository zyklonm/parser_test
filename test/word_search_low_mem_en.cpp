...
void print_node(struct trie_tree *node)
{
    if( node->children_node != NULL )
    {
	print_node(node->children_node);
    }

    if( node->next_node != NULL )
    {
	print_node(node->next_node);
    }
...
//    printf("%s : %d\n",node->word,node->number_of_value);
}
