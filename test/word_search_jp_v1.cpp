...
void print_node(struct trie_tree *node, int depth)
{
	...
	*(search_word + depth) = '\0' ;
	cout<<search_word<<" : "<<node->number_of_value<<endl;
        file_word_num <<search_word << " : " <<node->number_of_value<<endl;
	file_num << node->number_of_value<<endl;
    }
}
...
int main()
{
    ...
    int n = 0;
    int num = 0;
    file_word_num.open("jp_word_list.txt");
    file_num.open("jp_word_list_num.txt");
